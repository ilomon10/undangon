...
export const ImmichUploadWidget = ({ max = 1, onChange, folderTarget }) => {
  const [media, setMedia] = useState([]);

  const upload = useCallback(
    async ({ key, file }) => {
      let data = new FormData();
      data.append('deviceAssetId', `${file.name}-${Date.now()}`);
      data.append('deviceId', 'WEB');
      data.append('fileCreatedAt', new Date().toISOString());
      data.append('fileModifiedAt', new Date().toISOString());
      data.append('filename', file.name);
      data.append('assetData', file, file.name);
      let res = await immich.uploadAsset(data);
      // {
      //   assetData: file,
      //   fileName: generateId(),
...
export const ImmichUploadWidgetButton = ({ onSave, folderTarget }) => {
